...
function getModeColor(mode: string): string {
  ...
  if (mode === 'less' || mode === 'man') return 'var(--text-yellow)'
...
export default function ModeHUD({ mode, cwd, gitBranch, gitDirty, processName, tmuxSession, hudMode, onCycleHudMode }: ModeHUDProps) {
  ...
  return (
    <div
      className="flex items-center gap-4 px-4 border-t h-12 flex-shrink-0"
      style={{ backgroundColor: 'var(--bg-tertiary)', borderColor: 'var(--border-subtle)' }}
      role="status"
      aria-label="Terminal status and shortcuts"
    >
      {/* cwd */}
      <div className="flex items-center gap-1 font-jetbrains text-code-sm" style={{ color: 'var(--neon-cyan)' }}>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" />
        </svg>
        <span className="max-w-[200px] truncate" title={displayCwd}>{displayCwd}</span>
      </div>
      ...
      {gitBranch && (
        <>
          <div className="flex items-center gap-1 font-jetbrains text-code-sm" style={{ color: 'var(--neon-green)' }}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* Process */}
      <div className="flex items-center gap-1 font-jetbrains text-code-sm" style={{ color: mode.startsWith('vim') || mode === 'less' ? 'var(--status-purple)' : 'var(--text-secondary)' }}>
        <span>proc:</span>
      ...
      {tmuxSession && (
        <>
          <div className="flex items-center gap-1 font-jetbrains text-code-sm" style={{ color: 'var(--neon-blue)' }}>
      ...
      {/* Mode badge */}
      <motion.div
        className="px-2 py-0.5 rounded-full font-jetbrains text-[10px] font-semibold uppercase tracking-wider"
        style={{
          color: modeColor,
      ...
      <button
        ...
        className="ml-2 p-1 rounded-sm text-[var(--text-muted)] hover:text-[var(--neon-cyan)] hover:bg-[var(--bg-input)] transition-all"
